...
function buildSystemPrompt(mode, strictOutput = false) {
  const common = [
    ...
    "男主完整档案必须是‘完整背景档案’，不能只写碎片字段。至少覆盖：成长/家庭来源、教育与训练路径、职业变迁关键节点、能力形成机制、资源与人脉结构、财务与居住状态、日常行为模式、亲密关系与边界（需与用户硬约束一致）、当前核心冲突。",
...
function buildConsistencyPassPrompt(draft, selections, extraPrompt, mode, synthesizedConstraints = []) {
  return [
    ...
    "要求：仅输出修订后的最终正文，不要解释。",
    "必须满足：",
    "- 所有已选轴约束与补充提示词约束不得被违背。",
    "- 不得新增‘开场时刻场景锚点’章节。",
    "- MC不得命名；除开场片段外称‘她’，开场片段用第一人称‘我’。",
    "- 若补充提示词写了‘处男/无情感经历’等硬限制，必须严格兑现，不得出现反例。",
    "- ‘男主完整档案’必须是完整背景叙述：含成长来源、教育训练、职业变迁关键节点、能力形成机制、资源网络、当前生活结构与核心冲突，不得是碎片字段清单。"
    "已选轴：",
    selections.map((s) => `- ${s.axis}: ${s.option}${s.detail ? `（${s.detail}）` : ""}`).join("\n"),
